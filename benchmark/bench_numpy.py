...
class MPS:
    def __init__(self, tensors: list[NDArray], transposed: bool = False):
...
def contract_mps_mpos_mps_rowbyrow(start_mps: MPS, mpo_list: list[MPO], end_mps: MPS):
    ...
    for i in range(mps_mpo_count-2):
        cur_contr_tsr = np.tensordot(cur_contr_tsr, mpo_list[i][n-1], axes=([i+1],[0]))
    cur_contr_tsr = np.tensordot(cur_contr_tsr, end_mps_transp[n-1], axes=([mps_mpo_count-1],[0]))
    
    # Middle rows
    for j in range(1, n-1):
        cur_contr_tsr = np.tensordot(start_mps[n-1-j], cur_contr_tsr, axes=([2],[0]))
        for i in range(1, mps_mpo_count-1):
            cur_contr_tsr = np.tensordot(mpo_list[i-1][n-1-j], cur_contr_tsr, axes=([0,3],[i,i+1])).transpose(*(list(range(2,2+i)) + [0,1] + list(range(2+i,mps_mpo_count+1))))
        cur_contr_tsr = np.tensordot(cur_contr_tsr, end_mps_transp[n-1-j], axes=([mps_mpo_count-1,mps_mpo_count],[0,2]))

    # Last row
    cur_contr_tsr = np.tensordot(start_mps[0], cur_contr_tsr, axes=([1],[0]))
    for i in range(1, mps_mpo_count-1):
        cur_contr_tsr = np.tensordot(mpo_list[i-1][0], cur_contr_tsr, axes=([0,2],[0,1]))
    cur_contr_tsr = np.tensordot(cur_contr_tsr, end_mps_transp[0], axes=([0,1],[0,1]))

    return cur_contr_tsr.item()
...
def benchmark():
    ...
    for n in N_RANGE:
        ...
        expval_results.append((n, "manual-tn-numpy", expval, expval_time))
